...
export default function KPIDashboard() {
  ...
  const donePct = plans.length > 0 ? Math.round(plans.filter(p => p.status === 'done').length / plans.length * 100) : 0
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  {plans.length > 0 && (
                    <p className="text-sm text-brand-500 mt-0.5">{t('kpi.progress')}: {donePct}% ({plans.filter(p => p.status === 'done').length}/{plans.length})</p>
                  )}
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                {[
                  ...
                  <div key={label} className="bg-brand-50 border border-brand-200 rounded-xl p-3 text-center">
                    ...
                    <p className="text-[10px] text-brand-500 leading-tight">{desc}</p>
                  </div>
                ))}
              </div>
